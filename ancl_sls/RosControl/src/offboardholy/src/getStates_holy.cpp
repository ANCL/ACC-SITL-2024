...
PendulumAngles FixToPenAngles(double Lx,double Ly,double Lz);
...
PendulumAngles ToPenAngles(double Lx,double Ly,double Lz) { //x=base.x
    ...
    ROS_INFO("angles.alpha: %f", angles.alpha);
    ROS_INFO("cosbeta: %f", cosbeta);
...
PendulumAngles FixToPenAngles(double Lx,double Ly,double Lz){
    PendulumAngles angles;
    double L = sqrt(pow(Lx, 2) + pow(Ly, 2) + pow(Lz, 2));
    double halfpi = std::acos(0.0);

    //alpha (x-axis rotation)
    angles.alpha = halfpi - (std::acos(Lx/L));

    //beta (y-axis rotation)
    angles.beta = halfpi - (std::acos(Ly/L));
...
PendulumAngles FixToPenAngles2(double Lx,double Ly,double Lz){
    PendulumAngles angles;

    //alpha (x-axis rotation)
    angles.alpha = std::atan(Ly/Lz);

    //beta (y-axis rotation)
    angles.beta = std::atan(Lx/Lz);
...
void gazebo_state_cb(const gazebo_msgs::LinkStates::ConstPtr& msg){
    ...
    double Ly = (-loadpose.position.y) - (-quadpose.position.y) ;
    double Lz = (-loadpose.position.z) - (-quadpose.position.z) ;
    penangle = FixToPenAngles2( Lx, Ly, -Lz ); // in the paper the definition of n3 are opposite to the Z axis of gazebo
    sls_state1.alpha = penangle.alpha;
    sls_state1.beta = penangle.beta;
